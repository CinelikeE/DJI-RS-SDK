 ...
 * 文件中部被注释掉的代码为 pycrc 生成的独立测试程序，固件中未使用。
 */
 ...
 * 查表法使用的 CRC32 查找表（256 项，由 pycrc 生成）。
 */
...
crc32_t crc32_update(crc32_t crc, const void *data, size_t data_len)
{
    ...
    while (data_len--) {
        ...
        crc = (crc32_table[tbl_idx] ^ (crc >> 8)) & 0xffffffff; /* 查表值与 CRC 高位移位异或 */
        d++;                                                /* 指向下一个数据字节 */
    }
    return crc & 0xffffffff;                                /* 掩码到 32 位后返回 */
}
...
static int get_crc32_config(int argc, char *argv[])
{
    int c;
    int option_index;
...
static void print_crc32_params(void)
{
    char format[20];
...
int main32(int argc, char *argv[])
{
    ...
    get_crc32_config(argc, argv);
    crc = crc32_init();
    crc = crc32_update(crc, (void *)str, sizeof(str));
